...
class GeneralInputTab(QWidget):
    ...
    def take_fds_section(self):
        ...
        try:
            ...
            pass
        # Detach from the widget tree as well; otherwise the box stays a child
        # of the body and floats on top of the info sub-tabs.  The caller holds
        # the returned reference, so the widget (and its app-driven children)
        # stay alive and can be reparented into the FDS tab.
        box.setParent(None)
        box.hide()
        self._fds_box = None
        return box
