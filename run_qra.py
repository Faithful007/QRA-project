...
class QRAMainWindowPlus(qra.QRAMainWindow):
    ...
    def init_ui(self):
        ...
        # Add collapsible Fire Incident Scenario table under Traffic Volume.
        self._add_fire_incident_scenario_table()
    ...
    def _create_fds_tab(self):
        ...
        # "Generate FDS" sub-tab = the new road-tunnel FDS deck builder
        # (Python port of the HTML/JS calculator).  The app's original FDS
        # section is kept alive (referenced) so its widgets, the scenario->HRR
        # bindings and the built-in generator keep working in the background.
        self._legacy_fds_box = fds_box
        try:
            from fds_deck_panel import FDSDeckBuilderPanel
            gen_widget = FDSDeckBuilderPanel(
                project_dir_getter=lambda: getattr(self, "project_dir", None))
        except Exception as exc:
            print(f"[warning] FDS deck builder unavailable: {exc}")
            gen_widget = fds_box
            gen_widget.show()
            self._legacy_fds_box = None
        gen_scroll = QScrollArea()
        gen_scroll.setWidgetResizable(True)
        gen_scroll.setWidget(gen_widget)
    ...
    def _merge_split_into_traffic_volume(self):
        ...
        if veh is None or split is None:
            return
        try:
            # Match requested UI wording for direction rows.
            veh.setVerticalHeaderItem(0, QTableWidgetItem("+Dir (AADT)"))
            veh.setVerticalHeaderItem(1, QTableWidgetItem("-Dir (AADT)"))
    ...
    def _mirror_split_into_volume(self):
        ...
        try:
            ...
            for k, (_label, sr) in enumerate(self._SPLIT_MERGE_ROWS):
                r = self._merge_base + k
                if k == 0:
                    # First merged row is computed from Dir AADT * Occupants,
                    # not mirrored from the split table.
                    continue
        ...
        self._recompute_number_of_passenger_row()
        if hasattr(self, "_refresh_traffic_performance_table"):
            self._refresh_traffic_performance_table()
    ...
    def _num_from_table(self, tbl, row, col):
        from PyQt5.QtWidgets import QTableWidget
        it = QTableWidget.item(tbl, row, col)
        return self._to_float_text(it.text() if it is not None else "")
    ...
    def _recompute_number_of_passenger_row(self):
        """Row below Occupants: Number of passenger = Dir AADT * Occupants.

        Per-vehicle columns use (+Dir + -Dir) * Occupants. Total column is
        the summation across vehicle columns.
        """
        from PyQt5.QtWidgets import QTableWidget
        ...
        if tbl is None or tc is None or base is None:
            return
        ...
        try:
            ...
            for c in range(tc):
                dir_aadt = (
                    self._num_from_table(tbl, row_plus_dir, c)
                    + self._num_from_table(tbl, row_minus_dir, c)
                )
                occ = self._num_from_table(tbl, row_occupants, c)
                val = dir_aadt * occ
                total_sum += val

                it = QTableWidget.item(tbl, row_num_pass, c)
                if it is not None:
                    it.setText(self._fmt_num(val))

            it_total = QTableWidget.item(tbl, row_num_pass, tc)
            if it_total is not None:
                it_total.setText(self._fmt_num(total_sum))
        finally:
    ...
    def _add_traffic_total_column(self):
        ...
        *logical* column (so every existing index-based read in the app keeps
        ...
        try:
            ...
            # Ensure one-way/two-way toggles also refresh Total and dependent rows.
            grp = getattr(self, "tbi_traffic_btn_group", None)
            if grp is not None:
                grp.idClicked.connect(lambda _id: self._recompute_traffic_totals())
        ...
        try:
            for r in range(tbl.rowCount()):
                ...
                if not seen:
                    ...
                    # +Dir/-Dir totals are displayed as rounded AADT-like integers.
                    ti.setText(f"{round(s):.0f}")
                else:
                    ti.setText("%g" % s)
        finally:
            tbl.blockSignals(False)
    ...
    def _traffic_total_agg_rows(self):
        ...
        if base is None:
            return set()
        # base+0 is "Number of passenger" and is now computed per-column,
        # then summed in Total. Keep only the remaining aggregate rows here.
    ...
    def _add_fire_incident_scenario_table(self):
        """Append a collapsible Fire Incident Scenario table below Traffic Volume.

        Default state is hidden (collapsed). User can expand/collapse with a
        drop-arrow button and add/remove rows to resize the table.
        """
        from PyQt5.QtCore import Qt
        from PyQt5.QtGui import QColor
        ...
        if getattr(self, "fire_incident_table", None) is not None:
            return

        grp = self._find_group("Traffic Volume")
        if grp is None or grp.layout() is None:
            return

        host = QWidget()
        host_v = QVBoxLayout(host)
        host_v.setContentsMargins(0, 2, 0, 0)
        host_v.setSpacing(4)

        # Collapsible header with a drop-arrow.
        hdr = QHBoxLayout()
        self.fire_incident_toggle = QToolButton()
        self.fire_incident_toggle.setCheckable(True)
        self.fire_incident_toggle.setChecked(False)  # hidden by default
        self.fire_incident_toggle.setArrowType(Qt.RightArrow)
        self.fire_incident_toggle.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        self.fire_incident_toggle.setText("Fire Incident Scenario")
        self.fire_incident_toggle.setStyleSheet(
            "QToolButton{font-weight:bold;color:#b03a2e;padding:2px 4px;}")
        hdr.addWidget(self.fire_incident_toggle)

        self.traffic_perf_toggle = QToolButton()
        self.traffic_perf_toggle.setCheckable(True)
        self.traffic_perf_toggle.setChecked(False)  # hidden by default
        self.traffic_perf_toggle.setArrowType(Qt.RightArrow)
        self.traffic_perf_toggle.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        self.traffic_perf_toggle.setText("Traffic Performance")
        self.traffic_perf_toggle.setStyleSheet(
            "QToolButton{font-weight:bold;color:#1f618d;padding:2px 4px;}")
        hdr.addWidget(self.traffic_perf_toggle)

        self.annual_passenger_label = QLabel("Annual Passenger Count:")
        self.annual_passenger_label.setStyleSheet("font-weight:bold;color:#2c3e50;")
        hdr.addWidget(self.annual_passenger_label)
        self.annual_passenger_value = QLineEdit("0")
        self.annual_passenger_value.setReadOnly(True)
        self.annual_passenger_value.setAlignment(Qt.AlignCenter)
        self.annual_passenger_value.setMaximumWidth(150)
        self.annual_passenger_value.setStyleSheet(
            "QLineEdit{background:#eef2f7;border:1px solid #bdc3c7;font-weight:bold;color:#1f618d;}")
        hdr.addWidget(self.annual_passenger_value)

        self.person_km_label = QLabel("Person_Km/Yr :")
        self.person_km_label.setStyleSheet("font-weight:bold;color:#2c3e50;")
        hdr.addWidget(self.person_km_label)
        self.person_km_value = QLineEdit("0")
        self.person_km_value.setReadOnly(True)
        self.person_km_value.setAlignment(Qt.AlignCenter)
        self.person_km_value.setMaximumWidth(160)
        self.person_km_value.setStyleSheet(
            "QLineEdit{background:#eef2f7;border:1px solid #bdc3c7;font-weight:bold;color:#1f618d;}")
        hdr.addWidget(self.person_km_value)
        hdr.addStretch(1)
        host_v.addLayout(hdr)

        self.fire_incident_panel = QGroupBox("Fire Incident Scenario")
        self.fire_incident_panel.setStyleSheet(
            "QGroupBox{font-weight:bold;color:#b03a2e;border:1px solid #bdc3c7;"
            "margin-top:10px;border-radius:3px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:8px;padding:0 4px;}"
        )
        self.fire_incident_panel.setVisible(False)
        panel_v = QVBoxLayout(self.fire_incident_panel)
        panel_v.setContentsMargins(4, 4, 4, 4)
        panel_v.setSpacing(4)

        self.fire_incident_table = QTableWidget(11, 3)
        self.fire_incident_table.setHorizontalHeaderLabels(["Category", "Condition", "Value"])
        self.fire_incident_table.verticalHeader().setVisible(False)
        self.fire_incident_table.setAlternatingRowColors(False)
        self.fire_incident_table.setSelectionBehavior(QAbstractItemView.SelectItems)
        self.fire_incident_table.setSelectionMode(QAbstractItemView.SingleSelection)
        self.fire_incident_table.setStyleSheet(
            "QTableWidget{gridline-color:#bdc3c7;font-size:11px;border:1px solid #95a5a6;}"
            "QHeaderView::section{background:#eef2f7;border:1px solid #bdc3c7;padding:4px;font-weight:bold;}"
        )
        hh = self.fire_incident_table.horizontalHeader()
        hh.setSectionResizeMode(0, QHeaderView.Stretch)
        hh.setSectionResizeMode(1, QHeaderView.ResizeToContents)
        hh.setSectionResizeMode(2, QHeaderView.ResizeToContents)

        # Seed values inspired by the provided UI screenshot.
        rows = [
            ("Closure Time", "", "2", QColor("#ffff66")),
            ("Warning Broadcast Time", "", "2", QColor("#ffff66")),
            ("Fire Scenario", "Normal", "60.0", QColor("#ffff66")),
            ("Driving Speed", "Delay", "5.0", QColor("#ffff66")),
            ("Fire Position", "", "250.0", QColor("#ffff66")),
            ("Evacuees", "Delay Frequency", "1.00%", QColor("#00b0f0")),
            ("Fire Rate in Heavy-Vehicle Fires", "", "60.0%", QColor("#ff3333")),
            ("Fire Rate in General Accidents", "", "5.0%", QColor("#ff3333")),
            ("Escalation Rate in Car Fires", "", "5.0%", QColor("#ff3333")),
            ("Vehicle Weight in HV Fires", "Bus", "87.0%", QColor("#00b0f0")),
            ("Vehicle Weight in HV Fires", "Truck", "11.0%", QColor("#00b0f0")),
        ]
        ...
        def _mk_item(text, bg=None):
            it = QTableWidgetItem(text)
            it.setTextAlignment(Qt.AlignCenter)
            if bg is not None:
                it.setBackground(bg)
                # Keep contrast for dark/red-blue style cells.
                if bg.value() < 200:
                    it.setForeground(QColor("white"))
            # Make all numeric-valued cells editable.
            if _is_numeric_text(text) or text.strip() == "":
                it.setFlags(it.flags() | Qt.ItemIsEditable)
            else:
                it.setFlags((it.flags() | Qt.ItemIsEnabled | Qt.ItemIsSelectable) & ~Qt.ItemIsEditable)
            return it

        for r in range(self.fire_incident_table.rowCount()):
            cat, cond, val, clr = rows[r] if r < len(rows) else ("", "", "", None)
            self.fire_incident_table.setItem(r, 0, _mk_item(cat))
            self.fire_incident_table.setItem(r, 1, _mk_item(cond))
            self.fire_incident_table.setItem(r, 2, _mk_item(val, clr))

        self.fire_incident_table.resizeRowsToContents()
        panel_v.addWidget(self.fire_incident_table)

        # Expand/reduce controls (row count).
        ctl = QHBoxLayout()
        add_btn = QPushButton("+ Add Row")
        del_btn = QPushButton("- Remove Row")
        add_btn.setStyleSheet("QPushButton{padding:4px 8px;}")
        del_btn.setStyleSheet("QPushButton{padding:4px 8px;}")
        ctl.addWidget(add_btn)
        ctl.addWidget(del_btn)
        ctl.addStretch(1)
        panel_v.addLayout(ctl)

        def _add_row():
            r = self.fire_incident_table.rowCount()
            self.fire_incident_table.insertRow(r)
            self.fire_incident_table.setItem(r, 0, _mk_item(""))
            self.fire_incident_table.setItem(r, 1, _mk_item(""))
            self.fire_incident_table.setItem(r, 2, _mk_item("", QColor("#00b0f0")))

        def _del_row():
            r = self.fire_incident_table.currentRow()
            if r < 0:
                r = self.fire_incident_table.rowCount() - 1
            if r >= 0:
                self.fire_incident_table.removeRow(r)

        add_btn.clicked.connect(_add_row)
        del_btn.clicked.connect(_del_row)

        # Separate collapsible panel: Traffic Performance.
        self.traffic_perf_panel = QGroupBox("Traffic Performance")
        self.traffic_perf_panel.setStyleSheet(
            "QGroupBox{font-weight:bold;color:#1f618d;border:1px solid #bdc3c7;"
            "margin-top:10px;border-radius:3px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:8px;padding:0 4px;}"
        )
        self.traffic_perf_panel.setVisible(False)
        traffic_right_v = QVBoxLayout(self.traffic_perf_panel)
        traffic_right_v.setContentsMargins(4, 4, 4, 4)
        traffic_right_v.setSpacing(4)

        self.traffic_perf_table = QTableWidget(7, 6)
        self.traffic_perf_table.setHorizontalHeaderLabels([
            "Vehicle Type",
            "Vehicle Count",
            "Annual Veh-km",
            "Accident Rate\n(cases/1e8 VK)",
        ...
        self.traffic_perf_table.verticalHeader().setVisible(False)
        self.traffic_perf_table.setSelectionBehavior(QAbstractItemView.SelectItems)
        self.traffic_perf_table.setSelectionMode(QAbstractItemView.SingleSelection)
        self.traffic_perf_table.setStyleSheet(
            "QTableWidget{gridline-color:#bdc3c7;font-size:11px;border:1px solid #95a5a6;}"
            "QHeaderView::section{background:#eef2f7;border:1px solid #bdc3c7;padding:4px;font-weight:bold;}"
        )
        th = self.traffic_perf_table.horizontalHeader()
        ...
        th.setSectionResizeMode(5, QHeaderView.Stretch)
        ...
        def _mk_perf_item(text="", editable=False, bg=None):
            p = QTableWidgetItem(text)
            p.setTextAlignment(Qt.AlignCenter)
            if bg is not None:
                p.setBackground(bg)
            flags = p.flags() | Qt.ItemIsEnabled | Qt.ItemIsSelectable
            if editable:
        ...
        for r, nm in enumerate(row_names):
            self.traffic_perf_table.setItem(r, 0, _mk_perf_item(nm, editable=False))
            self.traffic_perf_table.setItem(r, 1, _mk_perf_item("", editable=False, bg=QColor("#ffff66")))
            self.traffic_perf_table.setItem(r, 2, _mk_perf_item("", editable=False, bg=QColor("#ffff66")))
            self.traffic_perf_table.setItem(r, 3, _mk_perf_item("", editable=(r < 4), bg=QColor("#f5f5f5")))
            self.traffic_perf_table.setItem(r, 4, _mk_perf_item("", editable=False))
            self.traffic_perf_table.setItem(r, 5, _mk_perf_item("", editable=False))

        # Spreadsheet-aligned default accident-rate constants (cases / 1e8 VK).
        self.traffic_perf_table.item(0, 3).setText("0.77")
        self.traffic_perf_table.item(1, 3).setText("1.52")
        self.traffic_perf_table.item(2, 3).setText("2.30")
        self.traffic_perf_table.item(3, 3).setText("2.30")

        self.traffic_perf_table.resizeRowsToContents()
        traffic_right_v.addWidget(self.traffic_perf_table)

        self._updating_traffic_perf = False
        self.traffic_perf_table.itemChanged.connect(self._on_traffic_perf_item_changed)

        def _toggle(opened):
            self.fire_incident_toggle.setArrowType(Qt.DownArrow if opened else Qt.RightArrow)
            self.fire_incident_panel.setVisible(opened)

        self.fire_incident_toggle.toggled.connect(_toggle)

        def _toggle_perf(opened):
            self.traffic_perf_toggle.setArrowType(Qt.DownArrow if opened else Qt.RightArrow)
            self.traffic_perf_panel.setVisible(opened)

        self.traffic_perf_toggle.toggled.connect(_toggle_perf)

        # Keep this table at about one-third of the upper Traffic Volume table width.
        self._sync_fire_incident_width()
        top_table = getattr(self, "tbi_veh_table", None)
        if top_table is not None:
            top_table.installEventFilter(self)
            top_table.cellChanged.connect(lambda *_: self._refresh_traffic_performance_table())

        tlen = getattr(self, "tbi_length", None)
        if tlen is not None and hasattr(tlen, "textChanged"):
            tlen.textChanged.connect(lambda *_: self._refresh_traffic_performance_table())

        self._refresh_traffic_performance_table()

        # Place both collapsible panels on the same row:
        # Fire Incident ~1/3, Traffic Performance ~2/3.
        panels_row = QHBoxLayout()
        panels_row.setContentsMargins(0, 0, 0, 0)
        panels_row.setSpacing(8)
        panels_row.addWidget(self.fire_incident_panel, 1)
        panels_row.addWidget(self.traffic_perf_panel, 2)
        host_v.addLayout(panels_row)
        grp.layout().addWidget(host)

    def _sync_fire_incident_width(self):
        veh = getattr(self, "tbi_veh_table", None)
        panel = getattr(self, "fire_incident_panel", None)
        perf_panel = getattr(self, "traffic_perf_panel", None)
        table = getattr(self, "fire_incident_table", None)
        if veh is None or panel is None or perf_panel is None or table is None:
            return

        # Use current upper-table width and keep lower tables at ~1/3 and ~2/3.
        base_w = max(veh.viewport().width(), veh.width(), 1)
        target_w = max(280, int(base_w / 3))
        perf_w = max(420, base_w - target_w - 12)
        panel.setMinimumWidth(target_w)
        panel.setMaximumWidth(target_w)
        perf_panel.setMinimumWidth(perf_w)
        perf_panel.setMaximumWidth(perf_w)
        table.setMaximumWidth(max(240, target_w - 10))

    def _update_annual_passenger_count(self):
        veh = getattr(self, "tbi_veh_table", None)
        base = getattr(self, "_merge_base", None)
        tc = getattr(self, "_veh_total_col", None)
        out = getattr(self, "annual_passenger_value", None)
        out_person_km = getattr(self, "person_km_value", None)
        if veh is None or base is None or tc is None or out is None:
            return

        try:
            it = veh.item(base + 0, tc)
            total_passengers = float((it.text() if it else "0").replace(",", "").strip() or 0)
        except ValueError:
            total_passengers = 0.0

        annual = total_passengers * 365.0
        out.setText(f"{int(round(annual)):,}")

        if out_person_km is not None:
            length_km = self._traffic_perf_length_km()
            person_km_yr = length_km * annual
            out_person_km.setText(f"{int(round(person_km_yr)):,}")

    def eventFilter(self, obj, event):
        from PyQt5.QtCore import QEvent
        if obj is getattr(self, "tbi_veh_table", None):
            try:
                et = event.type()
    ...
    def _on_traffic_perf_item_changed(self, item):
        if getattr(self, "_updating_traffic_perf", False):
            return
        if item is None:
            return
        # Recompute when accident-rate cells are edited.
        if item.column() == 3 and item.row() < 4:
            self._refresh_traffic_performance_table()

    def _traffic_perf_rate(self, row):
        tbl = getattr(self, "traffic_perf_table", None)
        if tbl is None:
            return 0.0
        it = tbl.item(row, 3)
        try:
            return float((it.text() if it else "0").strip())
        except ValueError:
            return 0.0

    def _traffic_perf_length_km(self):
        # Tunnel length source in UI is meters; convert to km.
    ...
    def _refresh_traffic_performance_table(self):
        from PyQt5.QtWidgets import QTableWidget

        pt = getattr(self, "traffic_perf_table", None)
        veh = getattr(self, "tbi_veh_table", None)
        tc = getattr(self, "_veh_total_col", None)
        if pt is None or veh is None or tc is None:
            self._update_annual_passenger_count()
            return

        def vnum(r, c):
            it = QTableWidget.item(veh, r, c)
            try:
                return float((it.text() if it else "0").strip())
            except ValueError:
                return 0.0

        counts = [vnum(0, c) + vnum(1, c) for c in range(tc)]

        # Grouping from sheet convention.
        c_pc = counts[0] if len(counts) > 0 else 0.0
        c_bus_trk_s = sum(counts[i] for i in (1, 2, 3) if i < len(counts))
        c_trk_ml = sum(counts[i] for i in (4, 5) if i < len(counts))
        c_special = counts[6] if len(counts) > 6 else 0.0

        c_all = c_pc + c_bus_trk_s + c_trk_ml + c_special
        c_heavy = c_bus_trk_s + c_trk_ml + c_special

        length_km = self._traffic_perf_length_km()
        days_per_year = 365.0

        vk_pc = c_pc * length_km * days_per_year
        vk_bs = c_bus_trk_s * length_km * days_per_year
        vk_ml = c_trk_ml * length_km * days_per_year
        vk_sp = c_special * length_km * days_per_year

        vk_all = vk_pc + vk_bs + vk_ml + vk_sp
        vk_heavy = vk_bs + vk_ml + vk_sp

        r_pc = self._traffic_perf_rate(0)
        r_bs = self._traffic_perf_rate(1)
        r_ml = self._traffic_perf_rate(2)
        r_sp = self._traffic_perf_rate(3)

        occ_pc = vk_pc * r_pc / 1e8
        occ_bs = vk_bs * r_bs / 1e8
        occ_ml = vk_ml * r_ml / 1e8
        occ_sp = vk_sp * r_sp / 1e8
        occ_all = occ_pc + occ_bs + occ_ml + occ_sp

        r_all = (occ_all * 1e8 / vk_all) if vk_all > 0 else 0.0

        self._updating_traffic_perf = True
        try:
            # Row 0..3 detailed groups
            data = [
                (0, c_pc, vk_pc, r_pc, occ_pc),
                (1, c_bus_trk_s, vk_bs, r_bs, occ_bs),
                (2, c_trk_ml, vk_ml, r_ml, occ_ml),
                (3, c_special, vk_sp, r_sp, occ_sp),
            ]
            for r, cnt, vk, rate, occ in data:
                pt.item(r, 1).setText(self._fmt_int_commas(cnt))
                pt.item(r, 2).setText(self._fmt_int_commas(vk))
                pt.item(r, 3).setText(f"{rate:g}")
                pt.item(r, 4).setText(self._fmt_occ(occ))
                pt.item(r, 5).setText(self._fmt_ret(occ and (1.0 / occ) or 0.0))

            # Row 4: all vehicle types (weighted rate)
            pt.item(4, 1).setText(self._fmt_int_commas(c_all))
            pt.item(4, 2).setText(self._fmt_int_commas(vk_all))
            pt.item(4, 3).setText(f"{r_all:g}")
            pt.item(4, 4).setText(self._fmt_occ(occ_all))
            pt.item(4, 5).setText(self._fmt_ret(occ_all and (1.0 / occ_all) or 0.0))

            # Row 5: heavy vehicle total
            pt.item(5, 1).setText(self._fmt_int_commas(c_heavy))
            pt.item(5, 2).setText(self._fmt_int_commas(vk_heavy))
            pt.item(5, 3).setText("")
            pt.item(5, 4).setText("")
            pt.item(5, 5).setText("")
            ...
            # Row 6: total veh-km
            pt.item(6, 1).setText("")
            pt.item(6, 2).setText(self._fmt_int_commas(vk_all))
            pt.item(6, 3).setText("")
            pt.item(6, 4).setText("")
            pt.item(6, 5).setText("")
        finally:
