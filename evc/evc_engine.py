...
class EVCParams:
    ...
    def total_occupants_vb(self, pcpkpl_override=None, mix_rate_override=None,
        ...
        if mix_ok and clavg > 0:
            ...
            n_occ_float  = n_cong_veh * avg_occ
    ...
    def _run_one(self, run_no: int, rng=None, timestep_cb=None) -> RunResult:
        ...
        if self.fdb is not None and self.fdb.is_loaded:
            ...
            for ti in range(1, len(times_fdb)):
                ...
                # 🔥 CO FED rate — Purser model with VB-calibrated RMV.
                #
                # Standard Purser walking-adult formula (RMV = 25 L/min):
                #     fed_co = CO^1.036 / 35000        per minute
                #
                # VB-faithful evacuation formula (RMV ≈ 49 L/min):
                #     fed_co = CO^1.036 / 18000        per minute
                #
                # The 18000 divisor reflects HEAVY EXERTION breathing rate
                # (≈49 L/min) during emergency evacuation. This was calibrated
                # jointly with the visibility-only walking-speed model (CO no
                # longer slows walking — it only accumulates FED). The two
                # changes are coupled: removing CO walk-slowdown made agents
                # walk faster through smoke, reducing their FED accumulation
                # time. To preserve total FED, the divisor was tightened from
                # 35000 (standard Purser) to 18000 — a 1.94× rate increase.
                #
                # Joint optimization against all 18 calibration scenarios
                # (020/030/100 MW × 6 fire positions at CONGEST/FV0):
                #   Mean |EV time error|: 4.8%
                #   Mean |EQF error| (VB EQF >= 5):     29.6%
                #   Mean EQF signed error (real cases): -1.9% (unbiased)
                #
                # At 100 MW (the high-FED regime where EQF dominates Risk
                # Index), per-scenario EQF errors are: P1 -12%, P2 -15%,
                # P3 -9%, P4 -24%, P5 -38%. The remaining gap is from VB's
                # FED distribution being more peaked than Python's (VB has
                # heavier tail at FED >= 0.5).
                _PURSER_DIVISOR = 18000.0
                fed_co   = np.where(co_arr > 0.0,
                                    (co_arr ** 1.036) / _PURSER_DIVISOR,
                                    0.0)
                ...
                # Accumulate FED only for active (non-escaped) AND not-yet-
                # incapacitated agents. Empirical analysis against VB output
                # shows that VB's FED distribution at 100 MW is much more
                # gradual (FED≥0.1 → FED≥1.0 drops from 185 to 114, a 38%
                # decline) while Python without this filter showed only an
                # 18% decline because once agents hit FED=1.0 their FED kept
                # accumulating, saturating at the cap. This filter matches
                # VB's behavior: once an agent is incapacitated, additional
                # exposure doesn't drive further FED accumulation in the
                # bucket-count sense. (The cap at _FED_CAP also bounds the
                # EQ Fatal sum.)
            ...
            while t_now_post < t_post_end_max:
                ...
                # FED rates — identical formulas to the FDB-loop block above.
                # Accumulate ONLY for not-yet-incapacitated agents. Once an
                # agent's FED reaches 1.0 it's clamped to 1.2 (the saturation
                # cap inside np.clip below), but they still keep walking.
                not_incap = fed_total < 1.0
                # Standard Purser CO FED with VB heavy-exertion RMV scaling
                # (divisor 20000 instead of 35000). See the FDB-loop comment
            ...
            fed_total = np.clip(fed_total, 0.0, _FED_CAP)
            ...
            for _t in times_fb[1:]:
                ...
                fed_rate = np.where(co_arr > 0, (co_arr ** 1.036) / 18000, 0.0)
        ...
        #
        # NOTE on coupling with _PURSER_DIVISOR (line ~1040): the divisor
...
#         # reported Evacuees count to match VB's spreadsheet output exactly.
#         # FED≥1.0 fatalities are still tracked separately in the `fed` array
#         # (last bucket) and in `eq_fatal` (sum of FED for the FED ≥ 0.5 cohort).
#         incapacitated = fed_total >= 1.0
#         survivors = ~incapacitated   # used for zone/FED accounting AND evacuee count
#         n_fatal = int(np.sum(incapacitated))
#         evacuees_count = max(0, int(n_occ) - n_fatal)
...
#         n_evac_zone = int(np.sum(survivors & zone1_mask))
#         fed_counts = [int(np.sum(fed_total >= th)) for th in self.FED_THRESHOLDS]
#         # 🔥 EQ Fatal — sum of FED over ALL agents (VB-faithful).
#         # The original flag note (and an earlier version of this code) claimed
#         # EQ Fatal = sum(FED for FED >= 0.5). That is WRONG. Empirical check
#         # against the VB FNCV_ROAD.xlsm output sheet 'EVC_Result_BF' shows:
#         #
#         #   100/CONG/FV0/P4: FED>=0.5 count = 25.2, EQ Fatal = 42.30
#         #   100/CONG/FV0/P5: FED>=0.5 count = 1.6,  EQ Fatal = 14.30
#         #   100/CONG/FV0/P6: FED>=0.5 count = 0.0,  EQ Fatal = 18.00
#         #
#         # EQ Fatal is NONZERO even when FED>=0.5 count is zero, decisively
#         # ruling out the "≥0.5 cohort" formula. For high-FED P1-P3 scenarios,
#         # VB's EQ Fatal matches a midpoint-bucket-sum of all FED brackets
#         # ([0.1, 0.2), [0.2, 0.3), ..., [1.0, ∞)) to within ±5%.
#         #
#         # The VB formula is therefore: EQ Fatal = Σ FED_i over all agents.
#         # The "≥ 0.5 filter" was a misreading of the VB spreadsheet — the
#         # FED>=0.5 column is a separate count, not the input to EQ Fatal.
#         #
#         # This change increases reported EQ Fatal substantially in mild-smoke
#         # scenarios (e.g. P5/P6 at 100MW where Python previously reported
#         # ~0 due to the cohort filter, but VB reports 14-18). The headline
#         # Risk Index for those scenarios will rise accordingly.
#         eq_fatal = float(np.sum(fed_total))
